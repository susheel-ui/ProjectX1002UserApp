    ...
    fun updateStatus(id:String,status:String,token: String){
        if(status.equals(Keywords.ACCEPTED.toString())){
            viewModelScope.launch {
                ...
                val result =  repo.updateRequestStatus(id = id, status = Keywords.ACCEPTED.toString(), token = "Bearer ".plus(token))
                if(result.isSuccessful && result.code() == 200){
                    _loading.postValue(false)
                    _request.postValue(result.body())
            ...
            viewModelScope.launch {
                _loading.postValue(true)
                val result =  repo.updateRequestStatus(id = id, status = Keywords.REJECTED.toString(), token = "Bearer ".plus(token))
                if(result.isSuccessful && result.code() == 200){
                    _loading.postValue(false)
                    _request.postValue(result.body())
                } else if(result.code() == 500 ){
